...
- []()
- [Массивы (!)](#Массивы)
- [Системные и Матеманические функции, Время](#Системные-и-матеманические-функции-Время)
- [Сохраненки и игровая Статистика](#Сохраненки-и-игровая-Статистика)
- [Картинки, Звуки, Ссылки](#Картинки-Звуки-Ссылки)

## Объявить локацию
Локации должны иметь свои уникальные названия, то есть названия локаций не должны повторяться в пределах кода игры.

**\имя локации**  
Название локаций не будет видно пользователю в процессе игры, это служебная информация для автора. Название локации (_как и именем переменной_) может быть сочетание:
> кириллических или латинских символов
> знаков нижнего подчеркивания
> цифр
> пробелов и точек
> Название локации должно содержать хотя бы один кириллический или латинский символ в нижнем регистре или знак нижнего`_`подчеркивания.
...
Примеры видов именований локаций, которые будут автоматически приведены движком к каноническому:
...
\конструктор....игр_12
\конструктор   .игр_12
\  .. конструктор.   ..  .игр___12  ....**  Все они будут приведены одному и тому же виду: **\конструктор.игр_12**
...
Названием локации может быть сочетанием:
> кириллических или латинских символов    
> знаков нижнего подчеркивания    
> цифр    
> пробелов    
...
Примеры правильного объявления локаций:
...
**Визуальный редактор Аперо: для обобщения списка локаций в древовидную структуру следует использовать точки в имени локации. Например, группировать локации в пределах регионов катры и/или смысловых частей игры.**
TODO: _Тем самым в списке локаций справа от редактора кода не будет длинного списка локаций._

### 
**\СТОП** (синоним "end") - немедленно остановить выполнение кода. Обрати внимани, первый символ `\` косая черта, с наклоном влево, он обязателен. Этот симво также называют "обратный слэш". Это специальный символ для платформы, чтобы она понимала, что это команда, а не обычный текст. Если слово **СТОП** в коде было бы написано без обратного слэша, то платформа просто вывела бы его пользователю на экран как обычный текст.

Чтобы понять, как работает команда **\СТОП**, выполним следующий пример:
```
Этот текст будет показан игроку.
\СТОП
А текст после команды стоп1 - нет.
```
Посмотрите на результат выполнения. Платформа показывает игроку первую строчку, потом встречает команду СТОП и прекращает выполнение программы.

Само слово `стоп1` тоже не будет видно пользователю в процессе игры, это служебная информация.
...
\(массив=(1;-0.46;"Текст")) - объявить массив,  эквивалентно 
...
**\(массив(5)=7)** - объявить элемент массива,  эквивалентно `\(массив.номер5=7)`, на переменную \(массив.количество) не влияет. Перебор элементов массива в цикле \ЦИКЛ (н<массив.количество) \(н=н+1)элемент\(н)=\(массив(н))[br] - если `н` не задано, то выполняется от 1 до массив.количество, при этом выводит с каждой новой строки элемент\(н) = (значение элемента).

## Системные и матеманические функции Время

**\(АПЕРО.время)** - Переменная содержит количество секунд с 1 декабря 2014 (Открытие общего доступа Аперо для создания текстовых онлайн по Московскому времени)
...
**\(АПЕРО.время("Y/m/d H i s"))** - результатом вывода будет `2017/08/13 18 03 02`. Если во второй параметр передать количество секунд с 1 декабря 2014, то будет выведено не текущее, а заданное время (в заданном формате)
...
**\(АПЕРО.случайно)**
Переменная содержит случайное дробное значение от 0 до 0.99999 (дробные числа через точку). Переменная выдает новое значение при каждом вызове. Используется для создания случайных событий в игре.
...
## Сохраненки и игровая Статистика
