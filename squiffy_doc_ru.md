...
_Русский перевод выполнен машиной. Некоторые выражения и понятия адаптированы для лучшего понимания русскоязычными пользователями, изменен порядок заголовков. Этот перевод находится в состоянии доработки.
Помощь с переводом и концепцией: **Enola**, **Oreolek**_

# Сцены и действия

Вы можете форматировать текст с помощью Markdown, а также частично использовать HTML. Чтобы создавать интерактивные истории, вам нужно использовать разметку и комманды движка Squiffy. В Squiffy есть два типа текстовых блоков, которые вы можете использовать:
...
Перемещение игрока на новую сцену приведет к **деактивации всех ссылок** на предыдущей сцене, поэтому используйте переход на новую сцену, когда игрок предпринял необходимые действия, чтобы продвинуться по сюжету.

## Сцены (**Sections**)

Названия сцен должны быть уникальными в рамках всей игры. Создайте сцену, используя двойные квадратные скобки, а затем двоеточие. Затем поместите ниже сам текст, описывающий эту сцену. Для перемещения игрока на нужную сцену из любого другого произвольного места игры используйте двойные квадратные скобки, в которые заключено имя целевой сцены.
```
...
Для создания активной ссылки действия внутри сцены используйте одиночные квадратные скобки, в которые заключите фразу или слово из описания сцены, это будет именем действия. После текста, описывающего текущую программируемую сцену, создавайте блоки реакции на действия, которые в этой сцене буду предусмотрены. Для этих блоков используйте одиночные квадратные скобки, а затем двоеточие. После этого, ниже сам текст реакции на действие. В одиночные квадратные скобки поместите имя действия, которое фигурирует на текущей сцене как имя действия. Блоки реакций на действия могут содержать ссылки на другие действия, и так далее. После того, как игрок нажимает на ссылку действия, эта ссылка становится неактивной, игрок может кликнуть на неё только один раз. При переходе в другую сцену и, потом, по возвращении когда-нибудь на эту прежнюю сцену, все ранее деактивированные ссылки станут снова активными, если не указано иное, например проверка трекером. 
```
